...
The quadratic complexity, O(N²), of the self-attention mechanism represents a fundamental mathematical barrier to the scalability of modern neural networks. This paper addresses this challenge from both an architectural and a methodological standpoint. Architecturally, we introduce and rigorously analyze the Attention-Free Modular Perception (AFMP) framework, a class of architectures with provably linear-time complexity. This is achieved through a principled design grounded in predictive processing and sparse message-passing on a static graph. Methodologically, our primary contribution is the formalization of the Latency-FLOPs Dichotomy: a fundamental discrepancy between the asymptotic complexity model of floating-point operations (FLOPs) and the empirical, wall-clock latency on contemporary hardware accelerators. Our scalability analysis provides empirical validation of AFMP's O(N) complexity, contrasting it sharply with the quadratic bottleneck of Transformer models. However, this analysis also reveals that while algorithmically efficient in FLOPs, AFMP's sparse, memory-bound operations are sub-optimal on hardware architectures optimized for dense matrix multiplication. These findings establish AFMP as a principled framework for scalable architectures and, critically, leverage the Dichotomy to posit that future advances in computational efficiency are contingent upon a co-design of algorithms and their underlying hardware execution models.
