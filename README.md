...
This paper presents a mathematical framework for modeling and analyzing efficient deep learning architectures beyond conventional attention mechanisms. We introduce Attention-Free Modular Perception (AFMP), a mathematically grounded architecture that achieves provable linear-time $O(N)$ complexity through an error-driven sparse routing mechanism inspired by predictive coding and graph-theoretic optimization. The framework formalizes AFMP as a bounded-degree sparse graph $G=(V,E)$, enabling an analytical proof of linear computational and memory scaling with respect to sequence length $N$. In parallel, we define and derive the Latency--FLOPs Dichotomy, a quantitative measure capturing the mathematical disconnect between theoretical computational cost (FLOPs) and empirical wall-clock latency on modern hardware. Through rigorous complexity analysis, lemma-based proofs, and empirical validation, we demonstrate that AFMP maintains linear-time efficiency while offering robust generalization through learned sparsity and hierarchical feedback. The proposed framework unifies algorithmic and hardware perspectives under a common analytical foundation, positioning AFMP as a blueprint for mathematically interpretable and computationally scalable neural architectures.
